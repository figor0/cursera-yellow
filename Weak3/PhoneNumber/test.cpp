...
void TestPhoneNumberConstructor()
{
	...
	}
	{
		...
		Assert(number.GetCountryCode() == "7"
					 && number.GetCityCode() == "981"
					 && number.GetLocalNumber() == "876-47-73",
					 "my number international: wrong answer : +"
					 + number.GetCountryCode() + '-' +number.GetCityCode() + '-'
					 + number.GetLocalNumber() + " right = +7-981-876-47-73");
	}
	{
		string input("+1-2-coursera-cpp");
		PhoneNumber number(input);
		Assert(number.GetCountryCode() == "1"
					 && number.GetCityCode() == "2"
					 && number.GetLocalNumber() == "coursera-cpp",
					 "my number international literal: wrong answer : +"
					 + number.GetCountryCode() + '-' + number.GetCityCode() + '-'
					 + number.GetLocalNumber() + " right = +1-2-coursera-cpp");
	}
}
...
{
	{
		...
		try {
			...
			Assert( erWhat.empty(), "Ecxeption case wrong answer: empty");
		}
	}
	{
		string input("+--");
		try {
			...
			string erWhat = ec.what();
			Assert( erWhat.empty(), "Ecxeption case wrong answer: " + input);
		}
	}
	{
